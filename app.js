/**
 * LOGIKA SISTEM DASHBOARD EWS HYDROVISION, SOSIAL MEDIA, & MQTT
 */
  ...
  if (loginLogoRow) {
    ...
    CONFIG.LOGOS.forEach((logo) => {
      loginLogoRow.insertAdjacentHTML(
        "beforeend",
        `<img src="${logo.src}" alt="${logo.alt}" title="${logo.alt}" class="h-8 sm:h-9 object-contain shrink-0 hover:scale-110 transition drop-shadow" onerror="this.style.display='none'">`
      );
    });
  }
  ...
  if (hdrLogoRow) {
    ...
    CONFIG.LOGOS.forEach((logo) => {
      hdrLogoRow.insertAdjacentHTML(
        "beforeend",
        `<img src="${logo.src}" alt="${logo.alt}" title="${logo.alt}" class="h-7 sm:h-8 object-contain shrink-0 hover:scale-110 transition drop-shadow" onerror="this.style.display='none'">`
      );
    });
  }
}

// Render Data Dosen, Tim, dan Sosial Media Tim
function renderTeamInfo() {
  // Render Link Kontak Resmi Tim (IG, TikTok, & Official Gmail)
  ...
  if (igLink) igLink.href = CONFIG.SOSIAL_MEDIA.INSTAGRAM_URL;
  if (tiktokLink) tiktokLink.href = CONFIG.SOSIAL_MEDIA.TIKTOK_URL;
  if (emailLink) {
    emailLink.href = `mailto:${CONFIG.SOSIAL_MEDIA.EMAIL_TIM}`;
    emailLink.title = `Kirim email ke ${CONFIG.SOSIAL_MEDIA.EMAIL_TIM}`;
  }

  // Render Dosen Pembimbing
  const dosenImg = document.getElementById("dosen-foto");
  if (dosenImg) dosenImg.src = CONFIG.DOSEN.FOTO;
  document.getElementById("dosen-nama").innerText = CONFIG.DOSEN.NAMA;
  document.getElementById("dosen-jabatan").innerText = CONFIG.DOSEN.JABATAN;
  document.getElementById("dosen-instansi").innerText = CONFIG.DOSEN.INSTANSI;

  // Render 5 Anggota Tim (Tanpa Email Perorangan)
    ...
    CONFIG.TIM.forEach((member) => {
      ...
      const cardHTML = `
        <div class="flex flex-col items-center text-center p-4 rounded-3xl border ${borderCard} shadow-lg transition hover:scale-[1.02]">
          <!-- Foto Ukuran Persegi 5x5 cm / 140-160px -->
  ...
  if (gsheetLink) {
    ...
    gsheetLink.onclick = (e) => {
      if (!CONFIG.SPREADSHEET_URL || CONFIG.SPREADSHEET_URL.includes("CONTOH")) {
        e.preventDefault();
        alert("Harap isi link Google Spreadsheet asli Anda di file config.js!");
      }
    };
  }
}
...
function startESPHeartbeatChecker() {
  ...
  heartbeatInterval = setInterval(() => {
    ...
    if (now - lastTelemetryTime > 6000) {
      espPing.className = "w-2.5 h-2.5 rounded-full bg-red-500";
...
function initMQTT() {
  ...
  client.onConnectionLost = () => {
    document.getElementById("mqtt-status-ping").classList.replace("bg-emerald-500", "bg-red-500");
    document.getElementById("mqtt-status-text").innerText = "Server Putus";
  };
  ...
  client.onMessageArrived = (message) => {
    if (message.destinationName === CONFIG.MQTT.TOPIC_TELEMETRY) {
      try {
        ...
        const espPing = document.getElementById("esp-status-ping");
        const espText = document.getElementById("esp-status-text");
        espPing.className = "w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse";
        espText.innerText = "ESP32 Online";
        espText.className = "text-xs font-bold text-emerald-400";

        // Update Kartu Telemetri
        document.getElementById("val-ph").innerText = data.ph.toFixed(2);
        document.getElementById("val-turb").innerText = data.turb.toFixed(1) + " NTU";
        document.getElementById("val-dosis").innerText = data.dosis.toFixed(2);
        document.getElementById("val-vbat").innerText = data.v_bat.toFixed(1) + " V";
        document.getElementById("val-vpan").innerText = (data.v_pan ? data.v_pan.toFixed(1) : "0.0") + " V";
        document.getElementById("val-vesp").innerText = (data.v_esp ? data.v_esp.toFixed(1) : "5.0") + " V";
  ...
  client.connect({
    onSuccess: () => {
      document.getElementById("mqtt-status-ping").classList.replace("bg-red-500", "bg-emerald-500");
      document.getElementById("mqtt-status-text").innerText = "Server OK";
      client.subscribe(CONFIG.MQTT.TOPIC_TELEMETRY);
    },
    onFailure: () => {
      document.getElementById("mqtt-status-text").innerText = "Gagal Konek";
    }
  });
}
